...
namespace Library
{
    ...
    BasicMaterial::BasicMaterial()
        : Material("main11"),
          INITIALIZATION(WorldViewProjection)
    {
    }

    DEFINE(BasicMaterial, WorldViewProjection)
    ...
    void BasicMaterial::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
    {
        ...
        if (mesh.VertexColors().size() > 0)
        {
            ...
            for (UINT i = 0; i < sourceVertices.size(); i++)
            {
                ...
                vertices.push_back(BasicMaterialVertex(XMFLOAT4(pos.x, pos.y, pos.z, 1.0f), color));
            }
        }
        else
        {
