...
namespace Library
{
	...
	void Grid::SetPosition(const XMFLOAT3& pos)
	{
		mPosition = pos;
	...
	void Grid::InitializeGrid()
	{
        ...
        {
            float pos = maxPosition - (i * adjustedScale);

            // Vertical line
			vertices[j] = BasicMaterialVertex(XMFLOAT4(pos, 0.0f, maxPosition, 1.0f), mColor);
            vertices[j + 1] = BasicMaterialVertex(XMFLOAT4(pos, 0.0f, -maxPosition, 1.0f), mColor);

            // Horizontal line
            vertices[j + 2] = BasicMaterialVertex(XMFLOAT4(maxPosition, 0.0f, pos, 1.0f), mColor);
            vertices[j + 3] = BasicMaterialVertex(XMFLOAT4(-maxPosition, 0.0f, pos, 1.0f), mColor);
        }
