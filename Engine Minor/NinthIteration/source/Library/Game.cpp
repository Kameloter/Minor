...
namespace Library
{
    ...
    Game::Game(HINSTANCE instance, const std::wstring& windowClass, const std::wstring& windowTitle, int showCommand)
        : mInstance(instance), mWindowClass(windowClass), mWindowTitle(windowTitle), mShowCommand(showCommand),
		  ...
		  mFeatureLevel(D3D_FEATURE_LEVEL_9_1), mDirect3DDevice(nullptr), mDirectContext(nullptr), mSwapChain(nullptr),
	...
	void Game::Shutdown()
    {
		...
		if (mDirectContext != nullptr)
        {
			mDirectContext->ClearState();
        }

		ReleaseObject(mDirectContext);
    ...
    void Game::InitializeDirectX()
    {
		...
		if (FAILED(hr = direct3DDeviceContext->QueryInterface(__uuidof(ID3D11DeviceContext1), reinterpret_cast<void**>(&mDirectContext))))
        {
            throw GameException("ID3D11Device::QueryInterface() failed", hr);
        }
		...
		mDirectContext->RSSetViewports(1, &mViewport);
    }
